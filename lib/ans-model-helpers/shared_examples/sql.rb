...
shared_examples_for "Ans::Model::Helpers::BelongsTo" do
  describe "belongs_to" do
    before do
      @target ||= Object.const_get(@scope_name.to_s.camelize).find item_を作成(@scope_name).id
      @item ||= item_を作成(model.to_s.underscore.to_sym, :"#{@scope_name}_id" => @target.id)
    end
...
shared_examples_for "Ans::Model::Helpers::HasOne" do
  describe "has_one" do
    before do
      @item ||= item_を作成(model.to_s.underscore.to_sym)
      @owner ||= Object.const_get(@scope_name.to_s.camelize).find item_を作成(@scope_name, :"#{model.to_s.underscore}_id" => @item.id).id
    end
    it "は、対象のモデルインスタンスを返す" do
...
shared_examples_for "Ans::Model::Helpers::HasOneThrough" do
  describe "has_one through" do
    before do
      @item ||= item_を作成(model.to_s.underscore.to_sym)
      @owner ||= Object.const_get(@scope_name.to_s.camelize).find item_を作成(@scope_name).id
      item_を作成(@through, :"#{model.to_s.underscore}_id" => @item.id, :"#{@scope_name}_id" => @owner.id)
    end
    it "は、対象のモデルインスタンスを返す" do
      @item.send(@scope_name).should == @owner
    end
